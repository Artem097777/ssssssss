...
class MenuScreen(Screen):
    ...
    def setup_ui(self):
        ...
        info_label = Label(
            text='Управление: Виртуальный джойстик - движение, Кнопка E - взаимодействие',
...
class SettingsPopup(Popup):
    def __init__(self, **kwargs):
        ...
        difficulty_layout.add_widget(Label(text='Сложность:', font_size='24sp'))  
        self.difficulty_label = Label(text='Нормальная', font_size='24sp')
...
class Joystick(Widget):
    def __init__(self, size=150, **kwargs):
        super().__init__(**kwargs)
        self.size_hint = (None, None)
        self.size = (size, size)
        self.pos = (50, 50)  # Позиция в левом нижнем углу
        
        # Внешний круг джойстика
        with self.canvas:
            Color(0.5, 0.5, 0.5, 0.5)
            self.outer_circle = Ellipse(pos=self.pos, size=self.size)
            
            Color(0.7, 0.7, 0.7, 0.7)
            self.inner_circle = Ellipse(
                pos=(self.pos[0] + self.size[0]/2 - 30, self.pos[1] + self.size[1]/2 - 30),
                size=(60, 60)
            )
    ...
    def on_touch_up(self, touch):
        if touch.id == self.touch_id:
            ...
            self.inner_circle.pos = (
                self.pos[0] + self.size[0]/2 - 30,
                self.pos[1] + self.size[1]/2 - 30
            )
            return True
        return super().on_touch_up(touch)
    
    def update_joystick(self, touch):
        # Вычисляем относительную позицию касания внутри джойстика
        ...
        if distance > 0:
            self.vector = [dx / max_distance, dy / max_distance]
        else:
            self.vector = [0, 0]
...
class GameWidget(Widget):
    def __init__(self, **kwargs):
        ...
        # Добавляем элементы управления для Android
    ...
    def setup_touch_controls(self):
        """Настройка сенсорного управления для Android"""
        # Виртуальный джойстик для движения
        self.joystick = Joystick(size=150)
        ...
        self.interact_button = Button(
            ...
            size_hint=(None, None),
            size=(80, 80),
            pos=(Window.width - 100, 50),
            background_color=(0.2, 0.6, 0.2, 0.8),
            font_size='24sp'
        )
        self.interact_button.bind(on_press=self.interact_with_npc)
        ...
        self.menu_button = Button(
            ...
            size_hint=(None, None),
            size=(120, 60),
            pos=(Window.width - 140, Window.height - 80),
            background_color=(0.8, 0.5, 0.2, 0.8),
            font_size='20sp'
        )
        self.menu_button.bind(on_press=self.show_pause_menu)
        self.add_widget(self.menu_button)
    
    def show_pause_menu(self, instance=None):
        """Показать меню паузы"""
        if self.game_screen:
    ...
    def create_floor(self):
        ...
        # Простой цветной пол (без текстуры)
        self.floor_rects = []
    ...
    def interact_with_npc(self, instance=None):
    ...
    def update(self, dt):
        # Движение игрока через джойстик
        if self.joystick.vector != [0, 0]:
            new_x = self.player_x + self.joystick.vector[0] * self.player_speed
            new_y = self.player_y + self.joystick.vector[1] * self.player_speed
        else:
        ...
        # Обновление позиций кнопок (на случай изменения размера окна)
        self.interact_button.pos = (Window.width - 100, 50)
        self.menu_button.pos = (Window.width - 140, Window.height - 80)
